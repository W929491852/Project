<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
		<title>盘点单详细信息</title>
		<link rel="stylesheet" href="../style/drp.css">

	</head>
	<body class="body1">
		<div align="center">
			<form name="checkVoucherForm" id="checkVoucherForm">
				<table width="95%" border="0" cellspacing="2" cellpadding="2">
					<tr>
						<td>&nbsp;
							
						</td>
					</tr>
				</table>
				<table width="95%" border="0" cellspacing="0" cellpadding="0"
					height="8">
					<tr>
						<td width="522" class="p1" height="2" nowrap>
							<img src="../images/mark_arrow_03.gif" width="14" height="14">
							&nbsp;
							<b>盘点单详细信息</b>
						</td>
					</tr>
				</table>
				<hr width="97%" align="center" size=0>
				<table width="95%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td width="8%" nowrap="nowrap">
							<div align="right">
								&nbsp;&nbsp;&nbsp;单号:&nbsp;
							</div>
						</td>
						<td width="7%" nowrap="nowrap">
							200706260001
						</td>
						<td width="13%" nowrap="nowrap">
							<div align="right">
								&nbsp;&nbsp;&nbsp;分销商代码:&nbsp;
							</div>
						</td>
						<td width="6%">
							1001
						</td>
						<td width="15%" nowrap="nowrap">
							<div align="right">
								<div align="right">
									分销商名称:&nbsp;
								</div>
							</div>
						</td>
						<td width="18%" nowrap="nowrap">
							北京医药股份有限公司
						</td>
						<td width="10%" nowrap="nowrap">
							&nbsp;&nbsp;&nbsp;录入日期:&nbsp;
						</td>
						<td width="23%" nowrap="nowrap">
							2007-06-17
						</td>
					</tr>
				</table>
				<hr width="97%" align="center" size=0>
				<table width="95%" border="1" cellspacing="0" cellpadding="0"
					align="center" class="table1">
					<tr>
						<td width="89" class="rd6">
							物料代码
						</td>
						<td width="116" class="rd6">
							物料名称
						</td>
						<td width="74" class="rd6">
							规格
						</td>
						<td width="136" class="rd6">
							型号
						</td>
						<td width="120" class="rd6">
							计量单位
						</td>
						<td width="103" class="rd6">
							差异数量
						</td>
					</tr>
					<tr>
						<td class="rd8">
							2001
						</td>
						<td class="rd8">
							青霉素
						</td>
						<td class="rd8">
							ccdd
						</td>
						<td class="rd8">
							ddd
						</td>
						<td class="rd8">
							盒
						</td>
						<td class="rd8">
							-10
						</td>
					</tr>
					<tr>
						<td class="rd8">&nbsp;
							
						</td>
						<td class="rd8">&nbsp;
							
						</td>
						<td class="rd8">&nbsp;
							
						</td>
						<td class="rd8">&nbsp;
							
						</td>
						<td class="rd8">&nbsp;
							
						</td>
						<td class="rd8">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td width="89" class="rd7">&nbsp;
							
						</td>
						<td width="116" class="rd7">&nbsp;
							
						</td>
						<td width="74" class="rd7">&nbsp;
							
						</td>
						<td width="136" class="rd7">&nbsp;
							
						</td>
						<td width="120" class="rd7">&nbsp;
							
						</td>
						<td width="103" class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td width="89" class="rd7">&nbsp;
							
						</td>
						<td width="116" class="rd7" height="13">&nbsp;
							
						</td>
						<td width="74" class="rd7">&nbsp;
							
						</td>
						<td width="136" class="rd7">&nbsp;
							
						</td>
						<td width="120" class="rd7">&nbsp;
							
						</td>
						<td width="103" class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
					<tr>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7" height="13">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
						<td class="rd7">&nbsp;
							
						</td>
					</tr>
				</table>
				<p>
					<input name="btnBack" type="button" id="btnBack"
						onClick="window.close()" value="关闭">
				</p>
			</form>
		</div>
	</body>
</html>
